...
//ここから陣内変更
    ...
    if (topicName.startsWith("$iothub/twin", strlen("$iothub/twin"))) {
      IOTC_LOG(F("deviceTwinGetStateCallback") );
      if( *topicName ) {IOTC_LOG(F("topicName %s"), *topicName );}
      if( *payload ) {IOTC_LOG(F("payload %s"), *payload );}
      deviceTwinGetStateCallback(topicName, payload, singletonContext);

//　想定　文字列：　"devices/"　で始まり、　method-name=BuzzerTemplate6jk:AlertOn　の:の後ろをメソッド名として取り出す。
//　:　は　 %3A　と変換されてる可能性
//以下　デバイスへのコマンドの場合
    } else if (topicName.startsWith("devices/", strlen("devices/"))) {
      int index = topicName.indexOf("method-name=", strlen("method-name=") , 0);
      if (index == -1) {
        IOTC_LOG(F("ERROR: corrupt C2D message topic0 => %s"), *topicName);
        return;
      }

      index = topicName.indexOf("%3A", 3 , index);
      if (index == -1) {
        IOTC_LOG(F("ERROR: corrupt C2D message topic1 => %s"), *topicName);
        return;
      }

      AzureIOT::StringBuffer methodName( (char*)*topicName + index + 3 ,topic_length-index-3);
      IOTC_LOG(F("methodName: %s"), *methodName);
...
//---------------------------------
      ...
      IOTC_LOG(F("ERROR: unknown twin topic: %s, msg: %s"), *topicName,
               msg_length ? msg : "NULL");
    }
  }
}
